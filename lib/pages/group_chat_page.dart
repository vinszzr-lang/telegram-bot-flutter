...
class _GroupChatPageState extends State<GroupChatPage> {
  ...
  Future<void> _sendText() async {
    ...
    try {
      ...
      final r = await api.sendGroupMessage(widget.session.token!, group['id'].toString(), text, replyToId: replyId);
      _merge([r]);
      await LocalCache.saveMessages('group_${group['id']}', messages);
      if (mounted) setState(() {});
      _bottom();
  ...
  Future<void> _sendFile(File file, String name, String type) async {
    ...
    final replyId = replyingTo?['id']?.toString();
    try {
      final r = await api.uploadGroupMedia(widget.session.token!, group['id'].toString(), file, type: type, replyToId: replyId);
      _merge([r]);
  ...
  Widget _bubble(Map<String, dynamic> m) {
    ...
    return Align(alignment: mine ? Alignment.centerRight : Alignment.centerLeft, child: GestureDetector(onLongPress: () => _startSelection(m), onTap: selected.isNotEmpty ? () => _toggleSelected(m) : null, onHorizontalDragEnd: (d) { if ((d.primaryVelocity ?? 0) > 250) _reply(m); }, child: bubble));
  }
