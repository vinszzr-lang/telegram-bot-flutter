...
class _HomePageState extends State<HomePage> with RouteAware {
  ...
  void _startPolling(){pollTimer?.cancel();pollTimer=Timer.periodic(const Duration(seconds:8),(_){if(!socket.connected){syncFast().whenComplete((){if(mounted && widget.session.token!=null && !socket.connected){socket.connect(Api.baseUrl,widget.session.token!);socket.on('message:new',_onMessage);socket.on('profile:updated',(_)=>syncFast());}});}});}
  ...
  Future<void> syncFast({bool forceLoading=false})async{if(syncing||widget.session.token==null)return;syncing=true;if(forceLoading&&mounted)setState(()=>loading=true);try{final data=await api.sync(widget.session.token!);final user=Map<String,dynamic>.from(data['user']??{});await widget.session.updateUser(user);final next=<String,Map<String,dynamic>>{};_mergeInto(next,data['contacts']);_mergeInto(next,data['inbox']);for(final e in next.entries){final marker=await LocalCache.readAt(e.key);final lastRaw=e.value['lastMessageAt']?.toString();final last=lastRaw==null?null:DateTime.tryParse(lastRaw)?.toUtc();if(marker!=null&&last!=null&&!last.isAfter(marker))e.value['unread']=0;}contactMap..clear()..addAll(next);
      unreadTotal = next.values.fold<int>(0, (sum, c) => sum + ((c['unread'] is num) ? (c['unread'] as num).toInt() : int.tryParse('${c['unread'] ?? 0}') ?? 0));
      if(mounted)setState(()=>loading=false);}on ApiException catch(e){if(e.status==403)_showBanned();if(mounted)setState(()=>loading=false);}catch(_){if(mounted)setState(()=>loading=false);}finally{syncing=false;}}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          if (unreadTotal > 0) _UnreadBanner(count: unreadTotal)
          else if (bannerText != null) _NotificationBanner(text: bannerText!, onClose: _clearBanner),
          Padding(
