...
class _ChatPageState extends State<ChatPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> sendText() async {
    ...
    try {
      ...
      final result = await api.sendMessage(widget.session.token!, username, text, replyToId: replyId);
  ...
  Future<void> _sendSticker(String path) async {
    try {
      ...
      if (!mounted) return;
      setState(() => sendingMedia = true);
      final result = await api.uploadMedia(widget.session.token!, username, file, type: 'sticker', replyToId: _replyingTo?['id']?.toString());
      _mergeMessages([result]);
      _replyingTo = null;
      final local = await _localMediaFile(Map<String, dynamic>.from(result));
      await local.parent.create(recursive: true);
  ...
  Widget _interactive(Map<String,dynamic> message, Widget child) {
    ...
    final reply=message['replyTo'] is Map ? Map<String,dynamic>.from(message['replyTo']) : null;
    Widget wrapped=GestureDetector(
      onLongPress:()=>_longSelect(message),
      onTap:_selectedIds.isNotEmpty?()=>_toggleSelect(message):null,
      onHorizontalDragEnd:(d){if((d.primaryVelocity??0)>250)_startReply(message);},
      child:active?Container(decoration:BoxDecoration(color:const Color(0x443EA6FF),borderRadius:BorderRadius.circular(14)),child:child):child,
    );
    if(reply!=null) return wrapped;
    return wrapped;
  }
