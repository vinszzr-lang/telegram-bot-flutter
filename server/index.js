...
const initial = {
  ...
  users: [], contacts: [], blocks: [], messages: [], hidden: [], admins: [], activity: [], groups: [], groupMembers: []
};
...
function groupMessageOut(m, req, viewer) { const u=getUser(m.senderUsername); const saved=viewer?db.contacts.find(c=>c.owner===viewer&&c.username===m.senderUsername):null; const out={...messageOut(m,req),senderName:saved?.name || (u?displayName(u):m.senderUsername),senderAvatarUrl:rewriteUploadUrl(req,u?.avatarUrl||''),senderVerified:!!u?.verified}; if(m.poll){out.poll={question:m.poll.question,options:m.poll.options.map(o=>({id:o.id,text:o.text,votes:o.votes||0})),myVote:m.poll.voters?.[viewer]||null};} return out; }
...
app.post('/api/groups/:id/messages',auth,(req,res)=>{
  ...
  db.messages.push(m); save(); emitGroupMessage(g.id,m,req); res.json(groupMessageOut(m,req));
});
