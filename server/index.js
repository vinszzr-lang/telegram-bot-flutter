...
function safeUser(u) {
  return { username:u.username, firstName:u.firstName, lastName:u.lastName, name:`${u.firstName||''} ${u.lastName||''}`.trim(), displayName:`${u.firstName||''} ${u.lastName||''}`.trim(), verified:!!u.verified, avatarUrl:u.avatarUrl||'' };
}
...
app.get('/',(req,res)=>res.json({name:'ChatWithU API',ok:true,version:'2.0.0'}));
...
app.post('/api/auth/register',async(req,res)=>{
  ...
  const u={username:un,firstName:String(firstName).trim(),lastName:String(lastName).trim(),passwordHash:await bcrypt.hash(String(password),10),verified:false,banned:false,avatarUrl:'',createdAt:now()};
...
app.patch('/api/admin/users/:username',adminAuth,async(req,res)=>{const u=getUser(req.params.username);if(!u)return res.status(404).json({message:'User tidak ditemukan'});for(const k of ['firstName','lastName'])if(req.body[k]!==undefined)u[k]=String(req.body[k]);if(req.body.verified!==undefined)u.verified=!!req.body.verified;if(req.body.banned!==undefined)u.banned=!!req.body.banned;if(req.body.password!==undefined&&String(req.body.password).length>=4)u.passwordHash=await bcrypt.hash(String(req.body.password),10);save();emitUser(u.username,'profile:updated',{user:safeUser(u)});if(u.banned)emitUser(u.username,'banned',{});res.json({...safeUser(u),banned:!!u.banned});});
