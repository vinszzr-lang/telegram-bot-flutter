...
const PUBLIC_BASE_URL = process.env.PUBLIC_BASE_URL || `http://cloudadp.rexzystr.my.id:${PORT}`;
...
app.get('/api/media/:id/download',auth,(req,res)=>{const m=db.messages.find(x=>x.id===req.params.id);if(!m||!(m.senderUsername===req.user.username||m.recipientUsername===req.user.username))return res.status(404).json({message:'Media tidak ditemukan'});const raw=String(m.url||m.mediaUrl||'');let filename='';try{filename=path.basename(new URL(raw, PUBLIC_BASE_URL).pathname);}catch{filename=path.basename(raw);}if(!filename)return res.status(404).json({message:'File media tidak ditemukan'});const file=path.join(UPLOAD_DIR,'media',filename);if(!fs.existsSync(file))return res.status(404).json({message:'File media sudah tidak tersedia'});return res.sendFile(file);});
...
app.post('/api/chats/:username/media',auth,mediaUpload.single('file'),(req,res)=>{const other=String(req.params.username);if(!getUser(other))return res.status(404).json({message:'User tidak ditemukan'});if(!req.file)return res.status(400).json({message:'File tidak valid'});const type=['image','video','sticker','file'].includes(req.body?.type)?req.body.type:'file'; const mime=String(req.file.mimetype||'').toLowerCase(); const ext=path.extname(req.file.originalname||'').toLowerCase(); const isImage=mime.startsWith('image/')||['.jpg','.jpeg','.png','.webp','.gif','.heic','.heif','.avif'].includes(ext); const isVideo=mime.startsWith('video/')||['.mp4','.m4v','.mov','.webm','.mkv','.3gp','.avi'].includes(ext); if((type==='image'||type==='sticker')&&!isImage) return res.status(400).json({message:'Lampiran bukan foto.'}); if(type==='video'&&!isVideo) return res.status(400).json({message:'Lampiran bukan video.'});const url=`${PUBLIC_BASE_URL}/uploads/media/${req.file.filename}`;const caption=String(req.body?.caption||'').trim().slice(0,500);const message=caption||req.file.originalname;const m={id:id(),type,message,url,mediaUrl:url,fileName:req.file.originalname,mimeType:req.file.mimetype,size:req.file.size,caption:caption||'',senderUsername:req.user.username,recipientUsername:other,status:'sent',createdAt:now(),updatedAt:now()};db.messages.push(m);save();emitUser(other,'message:new',m);emitUser(req.user.username,'message:new',m);res.json(m);});
app.post('/api/chats/:username/read',auth,(req,res)=>{const other=String(req.params.username);const ids=[];db.messages.forEach(m=>{if(m.senderUsername===other&&m.recipientUsername===req.user.username&&m.status!=='read'){m.status='read';m.updatedAt=now();ids.push(m.id);}});if(ids.length)save();emitUser(other,'message:read',{username:req.user.username,other,ids});res.json({ok:true,ids});});
...
app.get('/api/chats/:username/media',auth,(req,res)=>{const msgs=visibleMessages(req.user.username,String(req.params.username));res.json({media:msgs.filter(m=>m.type==='image'||m.type==='video'||m.type==='sticker'),docs:msgs.filter(m=>m.type==='file'),links:msgs.filter(m=>m.type==='text'&&/(https?:\/\/|www\.)/i.test(m.message||''))});});
...
io.on('connection',socket=>{const u=socket.user.username;socket.join(`user:${u}`);sockets.set(socket.id,u);socket.on('typing:start',({to}={})=>{if(to)emitUser(String(to),'typing',{from:u,typing:true});});socket.on('typing:stop',({to}={})=>{if(to)emitUser(String(to),'typing',{from:u,typing:false});});socket.on('message:read',({other}={})=>{if(!other)return;const ids=[];db.messages.forEach(m=>{if(m.senderUsername===String(other)&&m.recipientUsername===u&&m.status!=='read'){m.status='read';m.updatedAt=now();ids.push(m.id);}});if(ids.length)save();emitUser(String(other),'message:read',{username:u,other:String(other),ids});});socket.on('disconnect',()=>sockets.delete(socket.id));});
